...
module.exports = function (config) {
    ...
    const c = config.tasks.scss.split;
    ...
    return function () {
        ...
        stream._transform = function (file, encoding, callback) {
            ...
            if (file.isNull() || !c.ranges) {
                return callback();
            }

            if (!config.tasks.scss.split.filter.test(file.path)) {
                return callback();
            }
            ...
            var tasks = c.ranges.map(process);
            ...
            function rename(path, range) {
                return path.replace(
                    /(\.css)$/,
                    '.' + c.urlPrefix + range[0] + '-' + (range.length > 1 ? range[1] : 'inf') + '$1'
                );
            }

            function handleResult(range, result) {
                const css = result.css;
                if (css.length && css.length / source.length < c.minRatio && source.length - css.length > 3000) {
